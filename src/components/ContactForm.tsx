...
const ContactForm: FC = () => {
  ...
  const onClick = () => consentContext.actions.toggleConsent('contactForm');

  return (
    <div className="p-4 bg-slate-100 border-black border-2 rounded-md relative">
      {consentContext.categories.contactForm.accepted ? (
        <>
          {submitted
        ...
        <button className="aspect-square font-sans w-full" onClick={onClick}>
          Hyväksy yhteystietolomakkeen käyttö
        </button>
      )}
    </div>
  );
