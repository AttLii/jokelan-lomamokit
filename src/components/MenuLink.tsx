...
export default function MenuLink({
  ...
  const showIcon =
    showSubItems && 'subItems' in menuItem && menuItem.subItems.length > 0;
  const activeClass = useMemo(
    () => (pathname === menuItem.path ? 'font-semibold' : ''),
    [pathname, menuItem.path]
  );
  return (
    <div
      className={`${className} ${showIcon ? 'pr-6' : ''} group relative flex items-center justify-center`}
    >
      <Link
        className={`w-full font-sans hover:underline ${activeClass}`}
        href={menuItem.path}
        onClick={onClick}
      >
        {menuItem.title}
